...
이를 위의 $1.25$처럼 증명하려고 해보자. $M$이 accept하는 input을 쪼갰을 때 $M_1$이 먼저 accept하고 $M_2$가 accept하는 것을 보이면 된다. 근데 문제는 $M$이 어디서 input을 break 해야할지를 모른다는 것이다! 이 문제를 해결하기 위해 새로운 기술인 $\textbf{nondeterminism}$을 사용해야 한다. 이는 다음 장으로 연결된다.